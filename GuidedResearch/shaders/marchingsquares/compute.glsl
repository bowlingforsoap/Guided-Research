	...
	*/
	...
	void emitDebug(vec2 value) {
		imageStore(contour, ivec3(gl_GlobalInvocationID.xy, 0), vec4(value.x, value.y, 0.f, 0.f));
		imageStore(contour, ivec3(gl_GlobalInvocationID.xy, 1), vec4(value.x, value.y, 0.f, 0.f));
		imageStore(contour, ivec3(gl_GlobalInvocationID.xy, 2), vec4(value.x, value.y, 0.f, 0.f));
		imageStore(contour, ivec3(gl_GlobalInvocationID.xy, 3), vec4(value.x, value.y, 0.f, 0.f));
	}
	...
	*/
	...
	void main() {
		...
		// if (ur.x > domainUpperBound.x || ur.y > domainUpperBound.y) {emitDebug(ur);}
		// if (ul.x > domainUpperBound.x || ul.y > domainUpperBound.y) {emitDebug(ul);}
		// if (ll.x > domainUpperBound.x || ll.y > domainUpperBound.y) {emitDebug(ll);}
		// if (lr.x > domainUpperBound.x || lr.y > domainUpperBound.y) {emitDebug(lr);}
